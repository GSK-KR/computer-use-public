...
첫 화면에서 위챗 백업, 카카오톡 백업, 결과 보기 중 필요한 작업을 고릅니다.
여러 방을 한 번에 저장하려면 첫 화면의 위챗 통째 백업 또는 카카오톡 통째 백업을 누릅니다.
위챗 백업은 시작 화면, 왼쪽 메뉴, 결과 화면의 위챗 백업 버튼에서 다시 들어갑니다.
기본 더블클릭 사용에서는 값을 붙여 넣는 절차가 없습니다.
앱 상태나 문자 인식 확인은 준비가 안 될 때 안에 있습니다.
웹 화면은 하나입니다. 결과 보기 화면에서도 위챗 백업과 카카오톡 백업으로 돌아갈 수 있습니다.
통째 백업 화면에서는 앱 창과 왼쪽 목록 준비를 체크한 뒤 1. 전체 목록 확인과 2. 확인한 목록 백업을 순서대로 누릅니다. 앱이나 문자 인식이 막혀 있으면 화면에 보이는 앱 열기 또는 문자 인식 설정을 먼저 누릅니다.
바로 클릭하지 않고 전체 목록 확인으로 후보 방 이름을 먼저 보여 준 뒤, 후보가 맞을 때 확인한 목록 백업으로 이어집니다.
방 개수 상한 또는 페이지 상한에 도달했다는 안내가 보이면 전체 목록이 끝났다고 보장하기 어렵습니다.
전체 목록 확인 단계라면 완료 카드의 상한 늘려 전체 목록 다시 확인을 먼저 누릅니다.
가능한 상한을 이미 최대로 올린 상태에서 현재 후보만 저장해도 되면 현재 후보 백업을 누릅니다.
확인한 목록 백업 뒤라면 저장된 결과 보기와 확인 필요 방 보기로 빠진 방을 본 뒤 다시 백업합니다.
